...
namespace MonoFlashLib.Engine
{
    public class Colisions
    {
        ...
        private static Vector2 Bouncing(Rectangle movingObject, Rectangle staticObject)
        {
            ...
            var difX = tileSizeHalf + halfSizeWidth - Math.Abs(posX - itemX);
            var difY = tileSizeHalf + halfSizeHeight - Math.Abs(posY - itemY);
            var result = movingObject.Location.ToVector2();
            if (difX < difY)
            {
                if (posX < itemX)
