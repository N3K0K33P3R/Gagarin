...
namespace MonoFlashLib.Engine
{
    ...
    {
        public static void HandleConnection(List<NetCommand> sendBuffer, List<NetCommand> readBuffer,
            NetworkStream networkStream)
        {
            foreach (var data in sendBuffer)
            {
                var ClientRequestBytes = Encoding.UTF8.GetBytes(data.command);
            ...
            if (networkStream.DataAvailable)
            {
                ...
                var temp = response.Split(new[] {'|'}, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in temp) readBuffer.Add(new NetCommand(item));
            }
        }


        public static void HandleConnection(List<NetByteCommand> sendBuffer, List<NetByteCommand> readBuffer,
            NetworkStream networkStream)
        {
            foreach (var data in sendBuffer)
