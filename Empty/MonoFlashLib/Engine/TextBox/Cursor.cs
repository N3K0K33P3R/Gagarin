    ...
    public class Cursor
    {
        ...
        public int? SelectedChar
        {
            ...
            set
            {
                if (value.HasValue)
                {
                    if (value.Value != TextCursor) selectedChar = (short) value.Value.Clamp(0, textBox.Text.Length);
                }
                else
                {
                    selectedChar = null;
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            var x = textBox.Renderer.Area.X;
            var y = textBox.Renderer.Area.Y;

            var cp = GetPosition(x, y, TextCursor);
            if (selectedChar.HasValue)
            {
                var sc = GetPosition(x, y, selectedChar.Value);
                if (sc.X > cp.X)
