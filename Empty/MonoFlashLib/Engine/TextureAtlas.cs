...
namespace MonoFlashLib.Engine
{
    public class TextureAtlas
    {
        ...
        public TextureAtlas(Texture2D atlas, int offset = 0, int tilesetTileSize = 32, int tileWidth = 32,
            int tileHeight = 32)
        {
            this.offset = offset;
            this.atlas = atlas;
        ...
        public Rectangle[] Get4SideTextureRect(int gid)
        {
            ...
            for (var i = 0; i < 4; i++) rects[i] = GetTextureRect(gid * 4 + i + offset);
            return rects;
        }
        ...
        public Rectangle[][] GetAnimTextureRect(int gid, int rows, int columns)
        {
            ...
            for (var i = 0; i < rows; i++) rects[i] = new Rectangle[columns];
